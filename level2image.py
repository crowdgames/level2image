...
class GroupShapeStyleAction(argparse.Action):
    ...
    def __call__(self, parser, namespace, values, option_string=None):
        if len(values) > 3:
            parser.error(
                "argument %s: expected at most three arguments" % option_string
            )
...
parser = argparse.ArgumentParser(
    description="Create image from level file.", formatter_class=CustomHelpFormatter
)
parser.add_argument("levelfiles", type=str, nargs="+", help="Input level files.")

group = parser.add_mutually_exclusive_group(required=False)
group.add_argument(
    "--background-files", type=str, nargs="+", help="Input background images."
)
group.add_argument(
    "--background-suffix",
    type=str,
    help="Suffix to remove from filenames when looking for backgrounds.",
)
group.add_argument(
    "--background-none",
    action="store_true",
    help="Don't automatically use background images if present.",
)
...
parser.add_argument(
    "--suffix", type=str, help="Extra suffix to add to output file.", default=".out"
)
parser.add_argument(
    "--fmt",
    type=str,
    choices=FMT_LIST,
    help="Output format, from: " + ",".join(FMT_LIST) + ".",
    default=FMT_PDF,
)
parser.add_argument(
...
parser.add_argument(
    ...
    type=str,
    nargs="+",
    action=GroupShapeStyleAction,
    help="How to display the group GROUP; SHAPE from: "
    + ",".join(SHAPE_LIST)
    + "; STYLE from: "
    + ",".join(PATH_LIST)
    + " or "
...
parser.add_argument(
    "--viz-hide", type=str, metavar="GROUP", action="append", help="Hide a group."
)
parser.add_argument(
    "--viz-none",
    action="store_true",
    help="Hide all groups other than those displayed.",
)
parser.add_argument(
    "--viz-color",
    type=str,
    ...
    metavar=("GROUP", "COLOR"),
    action="append",
    help="Which color to display a group.",
)
parser.add_argument(
    "--no-avoid", action="store_true", help="Don't try to avoid previous edges on path."
)
parser.add_argument("--no-blank", action="store_true", help="Don't output blank tiles.")
parser.add_argument(
    "--tile-image-folder", type=str, help="Folder to look for tile images in."
)
parser.add_argument("--padding", type=int, help="Padding around edges.", default=0)
parser.add_argument(
    "--anim-delay", type=int, help="Frame delay for animation (in ms).", default=250
)
parser.add_argument(
    "--raster-scale", type=int, help="Amount to scale raster images by.", default=2
)
...
parser.add_argument(
    ...
    type=int,
    nargs=4,
    metavar=("MAX_X", "MAX_Y", "PAD_X", "PAD_Y"),
    help="Put multiple levels in one image; MAX_X: number of levels per row or -1 for unlimited; MAX_Y: number of levels per column or -1 for unlimited; PAD_X: padding between levels on each row; PAD_Y: padding between levels on each column",
)

group = parser.add_mutually_exclusive_group(required=False)
group.add_argument(
    "--cairosvg", action="store_true", help="Only try to use cairosvg converter."
)
group.add_argument(
    "--svglib", action="store_true", help="Only try to use svglib converter."
)
...
if args.cfgfile is None:
    args.cfgfile = os.path.join(
        os.path.dirname(os.path.realpath(__file__)), "cfg-default.json"
    )

with open(args.cfgfile, "rt") as cfgfile:
    cfg = json.load(cfgfile)

if args.background_files is not None and len(args.background_files) != len(
    args.levelfiles
):
...
def is_between(ra, ca, rb, cb, rc, cc):
    ...
    return (
        abs(
            distance(ra, ca, rb, cb)
            + distance(rb, cb, rc, cc)
            - distance(ra, ca, rc, cc)
        )
        < 0.01
    )
...
def svg_rect(r0, c0, rsz, csz, padding, sides, style, color, drawn):
    ...
    if style == RECT_BORDER:
        ...
        if top:
            ret += (
                '  <line x1="%.2f" y1="%.2f" x2="%.2f" y2="%.2f" style="%s" stroke-linecap="square"/>\n'
...
def svg_line(
    ...
    if to_arrow:
        if as_arc:
            ...
            rotate = math.degrees(
                math.atan2(y2 - (midy + adjust * orthy), x2 - (midx + adjust * orthx))
            )
        else:
            rotate = math.degrees(math.atan2(y2 - y1, x2 - x1))

        ret += (
            '  <g transform="translate(%.2f %.2f) rotate(%.2f)"><polygon points="0 0, -4 -2, -4 2" fill="%s"%s/></g>\n'
            % (x2, y2, rotate, color, opts_shape)
        )

    if as_arc:
        ret += (
            '  <path d="M %.2f %.2f Q %.2f %.2f %.2f %.2f" stroke="%s" stroke-linecap="round" fill="none"%s/>\n'
            % (x1, y1, curvex, curvey, x2, y2, color, opts_line)
        )
    else:
        ret += (
            '  <line x1="%.2f" y1="%.2f" x2="%.2f" y2="%.2f" stroke="%s" stroke-linecap="round"%s/>\n'
    ...
    try:
        ...
        def _svg2png(svg, svg_width, svg_height, svg_scale):
            return cairosvg.svg2png(
                ...
                output_width=svg_width * svg_scale,
                output_height=svg_height * svg_scale,
            )
...
def initialize_svglib():
    try:
        ...
        def _svg2png(svg, svg_width, svg_height, svg_scale):
            return reportlab.graphics.renderPM.drawToString(
                _svg2rlg(svg, svg_scale),
                fmt="PNG",
                dpi=72 * svg_scale,
                backend="_renderPM",
            )
...
def initialize_unsupported():
    def _svg2pdf(svg):
        print(
            "Unsupported conversion to pdf. Try installing packages for cairosvg or svglib."
        )
        sys.exit(-1)

    def _svg2png(svg, svg_width, svg_height, svg_scale):
        print(
            "Unsupported conversion to image. Try installing packages for cairosvg or svglib."
        )
        sys.exit(-1)
...
initializers = [
    (initialize_cairosvg, "cairosvg", not args.svglib),
...
if args.viz is not None:
    for group, shape, style in args.viz:
        ...
        for shape, style in shape_to_style.items():
            if (shape in [SHAPE_PATH, SHAPE_LINE] and style not in PATH_LIST) or (
                shape in [SHAPE_RECT, SHAPE_TILE] and style not in RECT_LIST
            ):
...
for li, levelfile in enumerate(args.levelfiles):
    ...
    def add_draw_data_old(draw_dict, line):
        ...
        if len(points_str) == 0:
            ...
            points = [
                tuple([float(el) for el in pt.strip().split()])
    ...
    if pngfilename is not None and os.path.exists(pngfilename):
        ...
        if args.tile_image_folder is not None:
            tile_image = PIL.Image.new(
                "RGBA", (content_width, content_height), (0, 0, 0, 0)
            )
        ...
        for linei, line in enumerate(lines):
            for chari, char in enumerate(line):
                ...
                if char in tilepng and tilepng[char] is not None:
                    tile_image.paste(
                        tilepng[char],
                        (x - args.padding, y + 1 - args.size_cell - args.padding),
                    )
                    ...
                    if char == "<":
                        ...
                        char = None
                        custom = (
                            '<path d="M %.2f %.2f L %.2f %.2f L %.2f %.2f" stroke="%s" stroke-width="1" stroke-linecap="round" fill="none"/>'
                            % (
                                x + gz * pth[0],
                                yo + gz * pth[1],
                                x + gz * 0.5,
                    ...
                    if char is not None:
                        inner_svg += (
                            '  <text x="%.2f" y="%.2f" dominant-baseline="middle" text-anchor="middle" fill="%s" style="fill-opacity:%.2f">%s</text>\n'
                            % (
                                x + 0.5 * args.size_cell,
                                y - 0.34 * args.size_cell,
                                clr,
                                1.0,
                                char,
                            )
                    ...
                    inner_svg += (
                        '  <rect x="%d" y="%d" width="%d" height="%d" style="stroke:none;fill:%s;fill-opacity:%.2f"/>\n'
                        % (
                            x,
                            y - args.size_cell + 1,
                            args.size_cell,
        ...
        for points in points_list:
            ...
            for rr, cc in points:
                if tile_style == RECT_BORDER:
                    sides = (
                        [rr - 1, cc] not in points,
                        [rr + 1, cc] not in points,
                        [rr, cc - 1] not in points,
                        [rr, cc + 1] not in points,
                    )
                else:
                    sides = None
                inner_svg += svg_rect(
                    rr, cc, 1, 1, args.padding, sides, tile_style, tile_color, drawn
                )
    ...
    svg_height += args.padding
    svg += (
        '<svg viewBox="0 0 %d %d" version="1.1" xmlns="http://www.w3.org/2000/svg" font-family="Courier, monospace" font-size="%.2fpt">\n'
    ...
    if args.fmt != FMT_GIF_ANIM:
        if args.stdout:
            ...
            outfilename = (
                str(pathlib.Path(levelfile).with_suffix("")) + args.suffix + ext
            )
...
if args.fmt == FMT_GIF_ANIM:
    outfilename = (
        str(pathlib.Path(anim_name).with_suffix("")) + args.suffix + ".anim.gif"
    )
    print(" - writing", outfilename)
