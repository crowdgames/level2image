...
with open(args.levelfile, 'rt') as levelfile:
...
elif args.outfile.endswith('.svg'):
    with open(args.outfile, 'wt') as outfile:
        outfile.write(svg)
elif args.outfile.endswith('.pdf'):
    import cairosvg
    with open(args.outfile, 'wb') as outfile:
        cairosvg.svg2pdf(svg, write_to=outfile)
else:
    raise RuntimeError('unknown extension for outfile: %s' % args.outfile)
